...
from ..preprocessing import binarization, segment
from ...helpers.file_helpers import add_white_padding 
from ...helpers.plot import display_imgs, display_img
...
def sub_predict(
    ...
    words, words_time = segment.parse_words(binary_img)
    process_time["parse_words"] = words_time["parse_words"]

    lines, lines_time = segment.group_words_by_lines(words)
    ...
    for line in lines:
        for (x, y, w, h) in line:
            ...
            binary = binarization.binarization(word_img)
            thin_img = cv2.erode(word_img, kernel, iterations = 1)

            white_img = cv2.bitwise_not(thin_img)
            word_img = add_white_padding(white_img, padding = 25)
            # resize = resize_img(thin_img, 32)
            display_img(thin_img)
            text += detector.predict(Image.fromarray(thin_img)) + " "
    ...
    #             thin_img = cv2.erode(word_img, kernel, iterations = 1)
    #             resize = resize_img(thin_img, 32)
    #             batch_img.append(resize)
            
    #         display_imgs(batch_img)
